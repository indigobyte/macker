...
import de.andrena.tools.macker.util.AnnotationUtil;
...
public class ParsedClassInfo extends AbstractClassInfo {
    ...
    private void parseMethodReferences(CtClass classFile) throws ClassParseException {
        for (CtBehavior behavior : classFile.getDeclaredMethods()) {
            ...
            addAnnotationReferences(AnnotationUtil.getAnnotations(behavior), ReferenceType.METHOD_SIGNATURE, behavior.getName(), methodAccess);
            for (String[] methodParameterAnnotation : AnnotationUtil.getParameterAnnotations(behavior)) {
    ...
    private void parseFieldReferences(CtClass classFile) throws ClassParseException {
        ...
        for (CtField field : fields) {
            ...
            addAnnotationReferences(AnnotationUtil.getFieldAnnotations(field), ReferenceType.FIELD_SIGNATURE, field.getName(), translateAccess(field.getModifiers()));
        }
    }

    private void addAnnotationReferences(String[] annotationObjects, ReferenceType referenceType, String memberName, AccessModifier accessModifier) throws ClassParseException {
        if (annotationObjects != null) {
            for (String annotationClassName : annotationObjects) {
    ...
    private void parseClassAnnotations(CtClass classFile) throws ClassParseException {
        addAnnotationReferences(AnnotationUtil.getClassAnnotations(classFile), ReferenceType.CLASS_ANNOTATION, "class_annotation", AccessModifier.PUBLIC);
    }
